...
export default function FoodOrderingInterface({ business }: FoodOrderingProps) {
  ...
  const categories = useMemo(() => {
    const uniqueCategories = new Set(
      menuItems
        .filter(item => item.category && item.is_available !== false)
    ...
    <div className="space-y-6 md:space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-12 border border-gray-200 rounded-md px-3 bg-white focus:border-stone-500 focus:outline-none"
              >
      ...
      {itemCount > 0 && (
        <button
          ...
          className="fixed bottom-6 right-6 bg-stone-700 hover:bg-stone-800 text-white rounded-full p-4 shadow-2xl transition-all duration-200 hover:scale-110 z-50 flex items-center gap-3"
        >
